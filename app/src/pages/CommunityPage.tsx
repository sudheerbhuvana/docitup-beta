...
import { feedAPI, entriesAPI } from '@/lib/api';
...
export default function CommunityPage() {
  ...
  const fetchFeed = async () => {
    try {
      const response = await feedAPI.getFeed({ limit: 20 });
  ...
  const filteredFeed = feed.filter(entry => {
    ...
    return (
      ...
      entry.userId?.fullName?.toLowerCase().includes(query)
    );
  });
      ...
      <div className="relative z-10 p-4 md:p-8">
        <div className="max-w-2xl mx-auto space-y-6">
          ...
          <div className="space-y-4">
            <h1 className="text-4xl md:text-5xl font-bold text-white">Feed</h1>
            ...
            <Card className="bg-zinc-900/80 backdrop-blur-xl border-zinc-800/50 shadow-2xl">
              <CardContent className="p-12 text-center">
                <p className="text-gray-400 mb-4">
                  {searchQuery ? 'No posts found matching your search' : 'No posts in your feed yet'}
                </p>
                {!searchQuery && (
                  <p className="text-sm text-gray-500">
                    Follow some users to see their posts here, or check out public posts in the community.
                  </p>
                )}
              </CardContent>
            </Card>
